...
void Flight_State_Task(u8 dT_ms,s16 *CH_N)
{
	...
	/*设置油门摇杆量*/
	thr_deadzone = (flag.wifi_ch_en != 0) ? 0 : 50;//假如不是wifi通道死区控制值为50
	fs.speed_set_h_norm[Z] = my_deadzone(CH_N[CH_THR],0,thr_deadzone) *0.0023f;
	fs.speed_set_h_norm_lpf[Z] += 0.5f *(fs.speed_set_h_norm[Z] - fs.speed_set_h_norm_lpf[Z]);//油门累加
	
	/*推油门起飞*/
	if(flag.unlock_sta)//解锁后
	{	
...
void Swtich_State_Task(u8 dT_ms)
{
	...
	if(sens_hd_check.of_ok || sens_hd_check.of_df_ok)
	{
		...
		//
		if(jsdata.of_qua>20 )//|| flag.flying == 0) //光流质量大于50 /*或者在飞行之前*/，认为光流可用，判定可用延迟时间为1秒
		{
